  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-crisp mt-1">Catch trending sounds before they peak.</p>
      </div>
      ...
      {!loaded && (
        <div className="rounded-xl border border-edge bg-surface-secondary p-5 sm:p-6 space-y-5">
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Your niche <span className="text-crisp">(optional)</span></label>
            ...
            <div className="flex gap-1">
              {[
                ...
                <button key={t.id} onClick={() => setActiveTab(t.id as typeof activeTab)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === t.id ? "bg-brand-600/20 text-brand-300 border border-brand-500/30" : "bg-surface-secondary text-zinc-400 border border-transparent hover:bg-surface-hover"}`}>
                  {t.label} <span className="text-2xs text-crisp ml-1">{t.count}</span>
                </button>
              ))}
            </div>
              ...
              <div key={i} className="rounded-xl border border-edge bg-surface-secondary p-4">
                <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
                  <div className="flex-1 min-w-0">
                    ...
                    {s.artist && <p className="text-xs text-crisp mt-0.5">by {s.artist}</p>}
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-zinc-400">{s.usesEstimate}</div>
                    <div className="text-2xs text-crisp">uses</div>
                  </div>
                </div>
