...
export default function FoundationAnalysisPage() {
  ...
  const { confirmSpend, spendDialog } = useSpendConfirm('foundation-analysis', 'Foundation Analysis')
    ...
    >
      <div className="space-y-6">
        <div>
          ...
          <p className="text-crisp mt-1">Set your strategic foundation. Captions, Viral Ideas, and Bio Optimizer will all build from this.</p>
        </div>
        ...
        <div className="rounded-xl border border-edge bg-surface-secondary p-5 sm:p-6 space-y-5">
          <div>
            ...
            <p className="text-xs text-crisp">Where you publish today and the basics of your reach.</p>
          </div>
        ...
        <div className="rounded-xl border border-edge bg-surface-secondary p-5 sm:p-6 space-y-5">
          <div>
            ...
            <p className="text-xs text-crisp">The pillars, audience, and direction that shape your content.</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Your 3 content pillars * <span className="text-crisp">(≤60 chars each)</span></label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select value={sophistication} onChange={(e) => setSophistication(e.target.value as Sophistication)} className="w-full rounded-lg bg-surface-tertiary border border-edge text-zinc-200 px-4 py-3 text-sm focus:outline-none focus:border-brand-400">
        ...
        <div className="rounded-xl border border-edge bg-surface-secondary p-5 sm:p-6 space-y-5">
          <div>
            ...
            <p className="text-xs text-crisp">What you&apos;re strong at and what&apos;s blocking you right now.</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Format strengths <span className="text-crisp">(select all that apply)</span></label>
            <div className="flex flex-wrap gap-2">
              {FORMATS.map((f) => (
        ...
        <div className="rounded-xl border border-edge bg-surface-secondary p-5 sm:p-6 space-y-5">
          <div>
            <h2 className="text-sm font-semibold text-zinc-100 mb-1">4. Evidence layer * <span className="text-crisp font-normal">— your top 1–3 posts</span></h2>
            <p className="text-xs text-crisp">Add links to your highest-performing {socialPlatformLabel(platform)} posts, then paste the caption or script so the audit has reliable evidence.</p>
          </div>

          {samples.map((s, i) => (
            <div key={i} className="rounded-lg border border-edge bg-surface-tertiary/40 p-4 space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">{socialPlatformLabel(platform)} post URL <span className="text-crisp">(recommended)</span></label>
                <input value={s.sourceUrl} onChange={(e) => setSample(i, { sourceUrl: e.target.value })} placeholder={`Paste a ${socialPlatformLabel(platform)} post link`}
                  className={`w-full rounded-lg bg-surface-tertiary border text-zinc-200 placeholder:text-crisp px-3 py-2.5 text-sm focus:outline-none ${sampleUrlIsInvalid(s) ? "border-red-500/60 focus:border-red-500/80" : "border-edge focus:border-brand-400"}`} />
              ...
              <div>
                ...
                <textarea rows={4} value={s.caption} onChange={(e) => setSample(i, { caption: e.target.value })}
                  ...
                  className="w-full rounded-lg bg-surface-tertiary border border-edge text-zinc-200 placeholder:text-crisp px-3 py-2.5 text-sm focus:outline-none focus:border-brand-400 resize-none" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-zinc-400 mb-1.5">Why it performed <span className="text-crisp">(metric)</span></label>
                  <input value={s.metric} onChange={(e) => setSample(i, { metric: e.target.value })} placeholder="e.g., 240K views, 8K saves, 1.2K shares"
                    className="w-full rounded-lg bg-surface-tertiary border border-edge text-zinc-200 placeholder:text-crisp px-3 py-2.5 text-sm focus:outline-none focus:border-brand-400" />
                </div>
                <div>
        ...
        <div className="flex items-center gap-3">
          <Button onClick={() => confirmSpend(handleAnalyze)} loading={loading} size="lg" className="w-full sm:w-auto">
            ...
            <CreditCost task="foundation-analysis" />
          </Button>
          {spendDialog}
          <p className="text-xs text-crisp hidden sm:block">Takes 30–60s. Saves your Creator Profile when complete.</p>
        </div>
      ...
      <div className="rounded-xl border border-brand-500/20 bg-surface-secondary p-5 space-y-4">
        ...
        <div>
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-left text-xs uppercase tracking-wider text-crisp"><th className="pb-2">Pattern</th><th className="pb-2">Example</th></tr></thead>
