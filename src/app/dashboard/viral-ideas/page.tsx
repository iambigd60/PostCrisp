...
function IdeaCard({ idea, index }: { idea: ViralIdea; index: number }) {
  ...
  return (
    <div className="rounded-xl border border-edge bg-surface-secondary hover:border-brand-400 transition-all">
      {/* Card header */}
      <div className="p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-xs text-crisp bg-surface-tertiary px-2 py-0.5 rounded-full">{idea.format}</span>
            <span className="text-xs text-crisp bg-surface-tertiary px-2 py-0.5 rounded-full">{idea.platform}</span>
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${engagementColor[idea.engagement] ?? 'text-zinc-400'}`}>
              {idea.engagement}
            </span>
          </div>
          <span className="text-crisp text-sm flex-shrink-0">#{index + 1}</span>
        </div>
        ...
        <div className="rounded-lg bg-surface-tertiary border border-edge px-4 py-3 mb-4">
          <p className="text-xs text-crisp mb-1">Hook</p>
      ...
      {/* Expanded details */}
      {expanded && (
        <div className="border-t border-edge p-5 space-y-4 animate-fade-in">
          <div>
            <p className="text-xs font-medium text-crisp mb-2 uppercase tracking-wider">Content Outline</p>
          ...
          <div>
            <p className="text-xs font-medium text-crisp mb-2 uppercase tracking-wider">Suggested Hashtags</p>
          ...
          <div className="flex gap-6 text-sm">
            <div>
              <p className="text-xs text-crisp mb-0.5">Best time to post</p>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-crisp mt-1">Get a stream of content ideas tailored to your niche and audience.</p>
      </div>
      ...
      <div className="rounded-xl border border-edge bg-surface-secondary p-5 sm:p-6 space-y-5">
        ...
        <div className="flex flex-wrap items-center justify-end gap-4">
          ...
          <div className="flex items-center gap-3">
            {!isPro && (
              <span className="text-xs text-crisp">{remaining} generations left today</span>
            )}
            <Button onClick={handleGenerate} loading={loading} size="lg">
              {loading ? 'Generating...' : '🚀 Generate Ideas'}
              <CreditCost task="viral-ideas" />
            </Button>
          </div>
        </div>
      </div>
