...
These repository results do not make Phase 0 complete. Task 4 still contains `NOT ENABLED`, `BLOCKED BY ACCESS`, and `REQUIRES AUTHORIZATION` controls. The restore drill has not run and still requires explicit authorization plus residual-cost acceptance. The Auth signature is now one read-only prepared-statement-compatible command, and the bounded credential-free launcher documented in the restore drill completed successfully against linked production and a migration-built fresh local clone stand-in. This closes the prior CLI multi-command execution defect without making the restore drill complete.
