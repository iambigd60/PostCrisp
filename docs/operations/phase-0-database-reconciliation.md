...
**Status:** Phase 0 is `BLOCKED`. Fresh local inventory-v2 now matches production exactly after an explicit local-only migration disables unused `pg_graphql`, and the local Auth/default-grant/database-backed gates pass. The new migration is not applied to production and requires an authorized dry-run/apply checkpoint. Captured production client-role grants, leaked-password protection, source/UI restore eligibility, the authorized restore drill, live Vercel/provider-control access, and independent exit review also remain unresolved.
