...
- a password-free global-role fingerprint covering role attributes, membership grantor/admin/inheritance/`SET ROLE` options, and settings scoped to all databases or the current database;
...
Metadata behavior follows the PostgreSQL 17 catalog reference for [`pg_auth_members`](https://www.postgresql.org/docs/17/catalog-pg-auth-members.html) and the current catalog/information-function references for [`pg_policy`](https://www.postgresql.org/docs/current/catalog-pg-policy.html), [`pg_proc`](https://www.postgresql.org/docs/current/catalog-pg-proc.html), and [`pg_get_functiondef`](https://www.postgresql.org/docs/current/functions-info.html).

Execute it against the source before authorization, again immediately before clone, and against the healthy clone. The credential-free launcher uses the authenticated [Supabase CLI query surface](https://supabase.com/docs/reference/cli/supabase-db-query), pins CLI `2.115.0`, and enforces a 45,000 ms deadline plus a separate 2,000 ms termination grace using Node's documented child-process APIs. On Windows it follows Node's documented `cmd.exe` launch pattern for `.cmd` shims, then uses `taskkill.exe /PID <validated-numeric-pid> /T /F` with `shell: false` to terminate the exact spawned tree. On POSIX it creates a detached process group and sends `SIGKILL` to the negative group PID. At timeout it discards and destroys captured streams/listeners before waiting for process-exit confirmation, so descendant-held pipes cannot extend the grace deadline. It accepts only `--linked`, `--local`, or a 20-lowercase-letter `--project-ref`. On success stdout is exactly one JSON object containing `auth_restore_signature`; CLI status and transaction lines are not included. Timeout and CLI errors emit only non-secret diagnostics and never return partial stdout/JSON. Store successful raw outputs only in the encrypted transient workspace:
...
For a fresh local clone stand-in, use `node scripts/phase0/capture-auth-restore-signature.mjs --local`. Exit 0 means the single read-only query completed and stdout parsed to the required object. Exit 1 means invalid arguments, timeout/tree cancellation, CLI failure, non-JSON output, or a missing signature object; do not use a partial capture. If tree termination cannot be confirmed within the grace deadline, the diagnostic says so and the capture remains failed.
